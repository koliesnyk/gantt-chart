var Gantt = (function () {
	...
	class Bar {
		...
		draw_phases() {
			...
			if (this.phases) {
				this.phases.forEach(phase => {
					const phase_start = date_utils.parse(phase.start);
					const phase_end = date_utils.parse(phase.end);
					const duration = date_utils.diff(phase_end, phase_start, 'hour') / this.gantt.options.step;
					const phase_width = this.gantt.options.column_width * duration;
		...
		draw_stones() {
			...
			if (this.stones) {
				...
				this.stones.forEach((stone, index) => {
					...
					if (textLength > check_label_width) {
						check_label_width = textLength;
					}

					if (((stone_start_p - date) < check_label_width) && (date > 0)) {
						check_date = index;
					}
				...
				this.stones.forEach((stone, index) => {
					...
					if (stone.end.length > 10) {
						stone.end = stone.end.substring(0, stone.end.indexOf('T'));
					}

					if (check_date && check_date > 0) {
						if (index == check_date || index == check_date - 1) {
							hide_label_class = ' hide-label';
						}
					}
					...
					if (index == 0) {
						createSVG('text', {
							x: this.phase_x(stone_start) - 15,
							y: text_y,
							innerHTML: 'F/F',
							class: 'stone stone-label stone-label-before',
							append_to: this.stones_group
						});
					}

					this.stones_group_inner = createSVG('g', {
						class: 'stones-group-inner' + hide_label_class + '',
						append_to: this.stones_group
					});

					createSVG('title', {
						innerHTML: stone.name + ' | ' + stone.end,
						class: 'stone stone-title',
						append_to: this.stones_group_inner
					});

					createSVG('polygon', {
						points: this.get_stone_polygon_points(stone_start_p, stone_y, stone_height).join(','),
						x: this.phase_x(stone_start),
						y: stone_y,
						width: 10,
						height: this.height,
						class: 'stone stone-polygon',
						append_to: this.stones_group_inner,
						title: 'test'
					});
					...
					createSVG('text', {
						x: this.phase_x(stone_start) + 15,
						y: text_y,
						innerHTML: stone.name,
						class: 'stone stone-label',
						append_to: this.stones_group_inner
					});
				});
			}
		...
		get_stone_polygon_points(stone_start_p, stone_y, stone_height) {
			return [
				stone_start_p - 8,
	...
	class Gantt {
		...
		make_grid_background() {
			...
			$.attr(this.$svg, {
				height: grid_height + this.options.padding + topOffset,
				width: '100%'
			});
		}
		...
		make_grid_header() {
			...
			createSVG('rect', {
				...
				y: 0,
				width: header_width,
				height: header_height - topOffset,
				class: 'grid-header',
				append_to: this.layers.grid
			});
		}
